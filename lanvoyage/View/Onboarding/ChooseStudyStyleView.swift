...
struct ChooseStudyStyleView: View {
    ...
    var body: some View {
        VStack{
            HStack{
                VoidStepsView(currentStep: 2, totalSteps: 2, tintColor: Color.violet500, disabledOpacity: 0.3)
                Spacer()
            }
            .padding(.bottom,8)
            HStack {
            ...
            LazyVGrid(columns: columns, alignment: .center, spacing: 12) {
                ForEach(StudyStyle.allCases, id: \.rawValue) { item in
                    SelectionChipView(label: item.rawValue, isSelected: Binding<Bool>(
            ...
            ForEach(TargetPeriod.allCases, id: \.rawValue) { item in
                HStack{
                    CheckCircleSelectionView(label: item.rawValue, isSelected: Binding<Bool>(
            ...
            GeometryReader { proxy in
                ...
                HStack(spacing: spacing) {
                    CustomButtonView(title: "이전", kind: .outline) {}
                        .frame(width: w1)
                        .padding(.leading, 0.2)
                    NavigationLink(destination: ChooseStudyStyleView()){
                        CustomButtonView(title: "다음", kind: (selectedStyles.isEmpty || selectedPeriod.count != 1) ? .disabled : .filled)
                            .frame(width: w2)
                            .padding(.trailing, 0.2)
                    }.disabled(selectedStyles.isEmpty || selectedPeriod.count != 1)
                }
            }
            .frame(height:49)
        }
